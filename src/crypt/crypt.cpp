...
std::string BinaryKeyToHex(const std::string& binary_key) {
  ...
  encoder.Attach(
      new CryptoPP::StringSink(hex_key));  // 将编码后的数据输出到字符串
  encoder.Put(reinterpret_cast<const byte*>(binary_key.data()),
              binary_key.size());
  encoder.MessageEnd();
  return hex_key;
}
...
std::string HexKeyToBinary(const std::string& hex_key) {
  ...
  decoder.Attach(
      new CryptoPP::StringSink(binary_key));  // 将解码后的数据输出到字符串
...
std::string GenECDHSharedSecret(const std::string& client_pri,
  ...
  CryptoPP::SecByteBlock pub(reinterpret_cast<const byte*>(server_pub.data()),
                             server_pub.size());
  CryptoPP::SecByteBlock priv(reinterpret_cast<const byte*>(client_pri.data()),
                              client_pri.size());
  if (!dh.Agree(shared, priv, pub)) {
    throw std::runtime_error("Failed to reach shared secret");
  }
  return std::string(reinterpret_cast<const char*>(shared.data()),
                     shared.size());
}
