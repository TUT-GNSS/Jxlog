...
class Executor {
  ...
  class ExecutorTimer {
   ...
   private:
    ...
    std::unordered_set<RepeatedTaskId>
        repeated_id_state_set_;  // 存放RepeatedTaskId的字典
  };
 ...
 public:
  ...
  void PostDelayedTask(const TaskRunnerTag& runner_tag, Task task,
                       const std::chrono::duration<R, P>& delta) {
    // 将对应线程池执行task任务 封装为一个func
    Task func =
        std::bind(&Executor::PostTask, this, runner_tag, std::move(task));
  ...
  template <typename R, typename P>
  RepeatedTaskId PostRepeatedTask(const TaskRunnerTag& runner_tag, Task task,
                                  const std::chrono::duration<R, P>& delta,
                                  uint64_t repeat_num) {
    Task func =
        std::bind(&Executor::PostTask, this, runner_tag, std::move(task));
  ...
  auto PostTaskAndGetResult(const TaskRunnerTag& runner_tag, T&& func,
                            Args&&... args)
    ...
    auto result = task_runner->RunRetTask(std::forward<T>(func),
